...
import rmi.Bank.*; // Import inner classes of Bank

/**
 * This class is another implementation of the RemoteBank interface. It uses a
 * database connection as its back end, so that client data isn't lost if the
 * server goes down. Note that it takes the database connection out of
 * "auto commit" mode and explicitly calls commit() and rollback() to ensure
...
public class RemoteDBBankServer extends UnicastRemoteObject
	...
	public synchronized void openAccount(String name, String password)
		...
		try {
			...
			s.executeUpdate("INSERT INTO accounts VALUES ('" + name + "', '"
					+ password + "', 0)");
			s.executeUpdate(
					"CREATE TABLE " + name + "_history (msg VARCHAR(80))");
			...
			throw new BankingException(
					"SQLException: " + e.getMessage() + ": " + e.getSQLState());
		}
	 ...
	 * existing account. If so, it returns the balance in that account. If not,
	 * it throws an exception. Note that this method does not call commit() or
	...
	public int verify(String name, String password)
		...
		try {
			s = db.createStatement();
			s.executeQuery("SELECT balance FROM accounts " + "WHERE name='"
	...
	public synchronized FunnyMoney closeAccount(String name, String password)
		...
		try {
			...
			s.executeUpdate("DELETE FROM accounts " + "WHERE name = '" + name
	...
	public synchronized void deposit(String name, String password,
			FunnyMoney money) throws RemoteException, BankingException {
		int balance = 0;
		Statement s = null;
		try {
			balance = verify(name, password);
			s = db.createStatement();
			// Update the balance
			s.executeUpdate("UPDATE accounts " + "SET balance = " + balance
					+ money.amount + " " + "WHERE name='" + name + "' "
					+ "  AND password = '" + password + "'");
			// Add a row to the transaction history
			s.executeUpdate(
					"INSERT INTO " + name + "_history " + "VALUES ('Deposited "
							+ money.amount + " at " + new Date() + "')");
			db.commit();
		} catch (SQLException e) {
			try {
				db.rollback();
	...
	public synchronized FunnyMoney withdraw(String name, String password,
			int amount) throws RemoteException, BankingException {
		int balance = 0;
		Statement s = null;
		try {
			balance = verify(name, password);
			...
			s.executeUpdate("UPDATE accounts " + "SET balance = "
					+ (balance - amount) + " " + "WHERE name='" + name + "' "
					+ "  AND password = '" + password + "'");
			// Add a row to the transaction history
			s.executeUpdate(
					"INSERT INTO " + name + "_history " + "VALUES ('Withdrew "
							+ amount + " at " + new Date() + "')");
			db.commit();
		} catch (SQLException e) {
			try {
	...
	public synchronized List getTransactionHistory(String name, String password)
			throws RemoteException, BankingException {
		Statement s = null;
	...
	public static void main(String[] args) {
		try {
			...
			Connection db = DriverManager.getConnection(database, user,
					password);
