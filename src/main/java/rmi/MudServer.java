...
/**
 * This class implements the RemoteMudServer interface. It also defines a main()
 * method so you can run it as a standalone program that will set up and
 * initialize a MUD server. Note that a MudServer maintains an entrance point to
 * a MUD, but it is not the MUD itself. Most of the interesting MUD
 * functionality is defined by the RemoteMudPlace interface and implemented by
 * the RemotePlace class. In addition to being a remote object, this class is
 * also Serializable, so that the state of the MUD can be saved to a file and
 * later restored. Note that the main() method defines two ways of starting a
 * MUD: one is to start it from scratch with a single initial place, and another
 * is to restore an existing MUD from a file.
 **/
public class MudServer extends UnicastRemoteObject
	...
	/**
	 * Start a MUD from scratch, with the given name and password. Create an
	 * initial MudPlace object as the entrance, giving it the specified name and
	 * description.
	 **/
	public MudServer(String mudname, String password, String placename,
	 ...
	 * remote objects looked up by name. It is simpler to do it this way than to
	 * use an actual Registry object. If the named place does not exist, it
	 * throws a NoSuchPlace exception
	 **/
	 ...
	 * remote method. The MudPlace() constructor calls it to register the new
	 * place it is creating.
	 **/
	...
	/**
	 * This remote method serializes and compresses the state of the MUD to a
	 * named file, if the specified password matches the one specified when the
	 * MUD was initially created. Note that the state of a MUD consists of all
	 * places in the MUD, with all things and exits in those places. The people
	 * in the MUD are not part of the state that is saved.
	 **/
	 ...
	 * name of a file containing the serialized and compressed state of an
	 ...
	 * place for the MUD, and a description of that entrance place. Besides
	 * creating the MudServer object, this program sets an appropriate security
	 * manager, and uses the default rmiregistry to register the the MudServer
